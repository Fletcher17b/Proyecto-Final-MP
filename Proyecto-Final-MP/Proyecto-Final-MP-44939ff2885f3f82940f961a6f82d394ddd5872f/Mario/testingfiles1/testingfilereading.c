...
int readproduct(FILE *a, struct product_template b) {

    struct product_template temp3;
    char comparador[20];
    char name1[100];char txtextention[] = ".txt";
    int found;
    printf("Desea buscar en el masterfile o en uno especifico? \n ");
    printf("Ingrese nombre del archivo que busca\n");
    gets(name1);
    limipiarbuffer();
    strcat(name1,txtextention);
    a = fopen(name1, "r");
    
    if (a == NULL) { printf("nada"); }
    ...
    gets(comparador);
    rewind(a);
    fread(&temp3, sizeof(struct product_template), 1, a);
    while(!feof(a)) {
